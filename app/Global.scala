...
// $COVERAGE-OFF$ Global object seems to not be available in tests
  ...
  override def onError(request: RequestHeader, ex: Throwable): Future[Result] = Future.successful {
    ...
    ex.getCause match {
      ...
      case x => {
        ...
        showError(500, "The server encountered an internal error", Some("Try again later, or report this to met.no"), request) // scalastyle:ignore magic.number
      }
    }
  }

  override def onBadRequest(request: RequestHeader, error: String): Future[Result] = Future.successful {
    showError(400, error, None, request) // scalastyle:ignore magic.number
  }
